  ...
  const idb = {
    ...
    open() {
      return new Promise((res, rej) => {
        const req = indexedDB.open('minibank_store', 5);
    ...
    clearAll() {
      return new Promise((res, rej) => {
        const stores = ['profile', 'wallet', 'tx', 'family'];
...
async function logout() {
  ...
  async function deposit(amt) {
    ...
    if (window.BalanceChainSDK && BalanceChainSDK.mirrorToMSL) {
      await BalanceChainSDK.mirrorToMSL({ userId: state.user.id, direction: 'in', amount: amt, currency: cur, ref: 'deposit' });
    }
    await pushTx({ ts: Date.now(), type: 'deposit', amount: amt, currency: cur, status: 'settled', actor: 'owner' });
    render(true, prev);
    renderInsights();
    renderHistory();
  }
  ...
  async function withdraw(amt) {
    ...
    await saveWallet();
    if (window.BalanceChainSDK && BalanceChainSDK.mirrorToMSL) {
      await BalanceChainSDK.mirrorToMSL({ userId: state.user.id, direction: 'out', amount: amt, currency: cur, ref: 'withdraw' });
    }
    await pushTx({ ts: Date.now(), type: 'withdraw', amount: amt, currency: cur, status: 'settled', actor: 'owner' });
    render(true, prev);
    renderInsights();
    renderHistory();
  }
  ...
  async function memberPay(amt, category) {
    ...
    if (window.BalanceChainSDK && BalanceChainSDK.mirrorToMSL) {
      await BalanceChainSDK.mirrorToMSL({ userId: state.user.id, direction: 'out', amount: amt, currency: baseCur, ref: 'member:' + m.id });
    }
    await pushTx({
      ts: Date.now(),
  ...
  async function ownerPay(amt, category) {
    ...
    setTimeout(async () => {
      if (window.BalanceChainSDK && BalanceChainSDK.mirrorToMSL) {
        await BalanceChainSDK.mirrorToMSL({ userId: state.user.id, direction: 'out', amount: amt, currency: cur, ref: 'merchant' });
      }
      const first = state.tx.find(t => t.status === 'pending');
      if (first) { first.status = 'settled'; await idb.put('tx', first); }
      render();
      renderInsights();
    }, 800);
  }
  ...
  function renderHistory() {
    ...
    txs.slice(0, 120).forEach(t => {
      ...
      const actorBadge = t.actor && t.actor !== 'owner'
        ...
        : '';
      const stClass = t.status === 'settled' ? 'ok' : (t.status === 'pending' ? 'pending' : 'err');
      const cur = t.currency || activeCur;
      const cat = t.category ? ` • ${prettyCategory(t.category)}` : '';
      d.innerHTML = `
        <div class="tx-head">
          <span class="tx-type">${t.type.toUpperCase()} • ${Number(t.amount).toFixed(2)} ${cur}${cat}</span>
