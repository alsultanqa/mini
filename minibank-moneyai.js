...
const categoriesDisplay = Object.entries(byCatBase)
  ...
  function renderInsights(modeOverride) {
    ...
    if (mode === 'chat') {
      container.innerHTML = `
        <div class="insight-card" style="grid-column:1/-1;">
          <div class="muted">Money AI – Live Chat Coach</div>
          <div id="chatWindow"
               style="margin-top:10px;height:320px;overflow-y:auto;padding:10px;
                      border-radius:14px;background:rgba(15,23,42,.9);
                      border:1px solid rgba(148,163,184,.35);">
          </div>

          <div style="display:flex;gap:6px;margin-top:10px;">
            <input type="text" id="chatInput" class="input"
                   placeholder="اسأل Money AI عن وضعك المالي، الأهداف، أو كيف تتحول من Rush إلى Rich..."
                   style="flex:1;">
  ...
  function wireChat() {
    ...
    function addMsg(sender, text) {
      ...
      if (sender === 'user') {
        ...
        bubble.style.background = 'rgba(15,23,42,.95)';
        bubble.style.border = '1px solid rgba(148,163,184,.4)';
    ...
    function addTyping() {
      ...
      t.style.background = 'rgba(15,23,42,.8)';
    ...
    async function sendFlow() {
      ...
      addTyping();
      try {
        const reply = await moneyAI_ChatReply(raw);
        removeTyping();
        addMsg('ai', reply);
      } catch (e) {
        removeTyping();
        addMsg('ai', 'حدث خطأ بسيط في الشرح، حاول مرة أخرى بعد ثوانٍ.');
    ...
    setTimeout(async () => {
      addTyping();
      const welcome = await moneyAI_ChatReply('__welcome__');
      removeTyping();
      addMsg('ai', welcome);
    }, 50);
  }
